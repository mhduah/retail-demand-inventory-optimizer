...
\mathrm{FillRate}
=
\frac{\mathrm{served\ units}}
{\mathrm{actual\ demand\ units}}
\times 100
```
...
```math
\mathrm{Operating\ cost}
=
5\times\mathrm{shortage\ units}
+
1\times\mathrm{leftover\ units}
```

Budget utilisation is:

```math
\mathrm{Budget\ utilisation}
=
\frac{\mathrm{purchase\ spend}}
{\mathrm{budget\ limit}}
\times 100
```
...
```math
\mathrm{Capacity\ utilisation}
=
\frac{\mathrm{allocated\ order\ units}}
{\mathrm{capacity\ limit}}
\times 100
```
